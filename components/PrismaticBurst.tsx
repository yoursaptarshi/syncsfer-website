...
const PrismaticBurst = ({
  ...
  useEffect(() => {
    ...
    // Initialize gradient texture with initial colors
    const initialColors = colors && colors.length > 0 ? colors : ['#9333EA', '#7C3AED'];
    const colorCount = initialColors.length;
    const colorData = new Uint8Array(colorCount * 4);
    for (let i = 0; i < colorCount; i++) {
      const [r, g, b] = hexToRgb01(initialColors[i]);
      colorData[i * 4 + 0] = Math.round(r * 255);
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uSpeed: { value: speed ?? 1 },
        uAnimType: { value: animationType === 'rotate3d' ? 1 : animationType === 'hover' ? 2 : 0 },
