...
const GooeyNav: React.FC<GooeyNavProps> = ({
  ...
  const handleClick = (e: React.MouseEvent<HTMLAnchorElement>, index: number) => {
    ...
    // Removed particle animation for glow effect
    if (textRef.current) {
    ...
    if (href.startsWith('#')) {
      ...
      if (targetElement) {
        ...
        const duration = Math.max(800, Math.min(1500, Math.abs(distance) * 0.8)); // 800ms to 1500ms based on distance
            ...
            {items.map((item, index) => (
              <li
                ...
                className={`rounded-full relative cursor-pointer transition-[background-color_color_box-shadow] duration-300 ease shadow-[0_0_0.5px_1.5px_transparent] ${
                  activeIndex === index ? 'active text-white' : 'text-gray-900'
                }`}
              >
                <a
                  href={item.href}
