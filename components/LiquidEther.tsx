  ...
  useEffect(() => {
    ...
    class CommonClass {
      ...
      init(container: HTMLElement) {
        ...
        this.pixelRatio = isMobile ? Math.min(window.devicePixelRatio || 1, 1.5) : Math.min(window.devicePixelRatio || 1, 2);
        this.resize();
        try {
          this.renderer = new THREE.WebGLRenderer({ antialias: !isMobile, alpha: true });
        } catch (error) {
          console.warn('WebGL not supported, LiquidEther disabled:', error);
          return;
        }
        // Always transparent
...
`;
    ...
    class Simulation {
      ...
      getFloatType() {
        ...
        const isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);
        // Use HalfFloatType on mobile for better performance
        return (isIOS || isMobile) ? THREE.HalfFloatType : THREE.FloatType;
      }
      createAllFBO() {
